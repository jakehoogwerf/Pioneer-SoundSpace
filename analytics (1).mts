...
export default async (req: Request, context: Context) => {
  ...
  if (req.method === "GET") {
    ...
    // Prefer the class from the URL path (/api/analytics/:class); fall back
    // to a ?class= query param so old links/bookmarks keep working.
    const classFilter = pathClass || url.searchParams.get("class");
...
export const config: Config = {
  path: ["/api/analytics", "/api/analytics/:class"],
  method: ["GET", "POST"],
};
